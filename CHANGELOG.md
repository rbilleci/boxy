...
|------|--------|-------------|--------|
| 2026-03-02 | 160-167 | Section 15: Core enhancements — sp_topics__resize_partitions, sp_topics__cache_invalidate, sp_subscriptions__subscribe_v4 (start from watermark/position-0), docs/lease-distribution.md (Hamilton apportionment), docs/evaluations/ (async-jdbc, goharvest, bento-patterns, mysql-jdbc-tuning) | TBD |
| 2026-03-02 | 154-159 | Section 14: Client SDKs — Worker/Consumer API (Worker, WorkerConfig, EventHandler, WorkerMetrics with thread pool + back-pressure + Micrometer), sdk scaffolds for Go/Python/Rust/.NET/JavaScript | TBD |
| 2026-03-02 | 150-153 | Section 13: Branch hygiene — docs/release-process.md, scripts/cleanup-stale-branches.sh, .github/branch-protection.md, .github/workflows/release.yml (GitHub Releases automation) | TBD |
| 2026-03-02 | 146-149 | Section 12: Packaging — root pom.xml metadata (name/description/license/scm/developers), Maven Central + GitHub Packages distribution config, GPG signing plugin, Maven staging plugin, docker/ evaluation environment (Dockerfile + docker-compose), docs/release-process.md | TBD |
