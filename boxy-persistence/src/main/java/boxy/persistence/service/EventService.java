...
public class EventService {
    ...
    private final Cache<TopicKey, Topic> topicCache = Caffeine.newBuilder()
            .expireAfterWrite(Duration.ofSeconds(5))
            .maximumSize(1000)
            .build();
    private final Cache<PartitionKey, Partition> partitionCache = Caffeine.newBuilder()
    ...
    public void publish(String tenant, String topicName, String key, String data) {
        var topic = resolveTopic(tenant, topicName);
        var partitionNumber = Math.floorMod(key.hashCode(), topic.partitions());
        var partition = resolvePartition(topic.id(), partitionNumber);
    ...
    private Topic resolveTopic(String tenant, String name) {
        var key = new TopicKey(tenant, name);
    ...
    private Partition resolvePartition(long topicId, int partitionNumber) {
        var key = new PartitionKey(topicId, partitionNumber);
