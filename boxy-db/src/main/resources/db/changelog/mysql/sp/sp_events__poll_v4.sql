-- sp_events__poll  (v4 — configurable lease lock duration from boxy_config)
--
-- === Item #90: Make consumer lease lock duration configurable ===
--   v3 hardcoded INTERVAL 3 SECOND for the lease lock duration.
--   v4 reads 'lease.lock.seconds' from the boxy_config table, falling back
--   to 3 seconds if the key is absent.  The config can be updated at runtime:
--     UPDATE boxy_config SET config_value = '5' WHERE config_key = 'lease.lock.seconds';
--   The change takes effect on the next poll call (no procedure reload needed).
--
--   All other behaviour is identical to v3 (EXIT HANDLER, adaptive polling,
--   configurable batch size, subscription stats update).
...
BEGIN
    ...
    DECLARE v_lease_lock_sec  INT DEFAULT 3;
    ...
    -- ------------------------------------------------------------------
    SELECT COALESCE(MAX(CAST(config_value AS SIGNED)), 3)
    ...
    UPDATE consumer_leases l
      ...
      JOIN cursors c ON c.id = l.cursor_id
       SET l.consumer_id  = p_consumer_id,
           l.locked_until = DATE_ADD(v_now, INTERVAL v_lease_lock_sec SECOND),
    ...
    -- ------------------------------------------------------------------
    -- Step 8: Return metadata (result set 2).
    -- ------------------------------------------------------------------
