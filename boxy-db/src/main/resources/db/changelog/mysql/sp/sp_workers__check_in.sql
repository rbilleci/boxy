...
BEGIN
    ...
    -- HEARTBEAT
    CALL sp_workers__heartbeat(p_worker_id, p_consumer_group_id, p_weight, v_status);
    CALL sp_workers__gc(p_consumer_group_id);
    CALL sp_consumer_groups__refresh_metrics(p_consumer_group_id);

    -- HANDLE WORKER ACCEPTANCE
    IF v_status = 'ACCEPTED' THEN

        CALL sp_workers__refresh_metrics(p_worker_id, p_consumer_group_id);
        CALL sp_workers__heartbeat(p_worker_id, p_consumer_group_id, p_weight, v_status);
       ...
       -- IDEAL W/SLACK OF 10%
        ...
        IF (v_leases_active > v_leases_max) THEN
            CALL sp_leases__release_excess(p_worker_id, p_consumer_group_id, v_leases_active - v_leases_max);
        ELSEIF (v_leases < v_leases_target) THEN
            CALL sp_leases__acquire(p_worker_id, p_consumer_group_id, v_leases_target - v_leases_active);
        END IF;
    END IF;
